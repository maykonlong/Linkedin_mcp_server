...
console.log(`=== DESVINCULAR COMPETÊNCIA: ${SKILL_NAME} ===`);
...
try {
  ...
  console.log(`[PASSO 4] Procurando a experiência: "${TARGET_EXPERIENCE}" para desvincular...`);
  ...
  if (checkboxContainer) {
      // Validação: verificar se já está marcado antes de tentar desmarcar
      const isChecked = await checkboxContainer.getAttribute('aria-checked');
      if (isChecked === 'true') {
          console.log('Competência encontrada e vinculada. Clicando para desvincular...');
          await checkboxContainer.click();
          await page.waitForTimeout(1000);
      } else {
          console.log('⚠️ A competência já está desvinculada dessa experiência.');
      }
  } else {
  ...
  await page.waitForTimeout(4000);
  console.log(`🎉 Vínculo da competência "${SKILL_NAME}" removido com sucesso!`);
